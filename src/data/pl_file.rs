...
impl PlFile {
    pub(crate) fn read_or_create() -> Result<Self> {
        ...
        if file_path.exists() {
            Ok(Self::lock_and_read(&file_path)?.1)
        } else {
    ...
    fn save(&mut self) -> Result<()> {
        ...
        if self.o_transient.is_none() {
            return Err(anyhow!("save: cannot save because password is not set"));
        }

        if !self.stored.readable.bundles.is_storable() {
            return Err(anyhow!("save: not all secrets are in correct state"));
        }
