...
use egui::{Color32, Context, Key, Modal, RichText, Sides, TextEdit};

#[allow(clippy::too_many_lines)]
pub fn change_password(pw: &mut Pw, pl_modal: &mut PlModal, pl_file: &mut PlFile, ctx: &Context) {
    Modal::new("change_password".into()).show(ctx, |ui| {
        let mut go_for_it = false;

        title_of_modal(None, &t!("Change the password for the prolock file"), ui);
